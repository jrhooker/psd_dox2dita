<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:variable name="files" select="/doxygen/compounddef[@kind = 'file'][(string-length(normalize-space(.)) -  string-length(normalize-space(compoundname))) &gt; 0]"/>
    <xsl:variable name="structures" select="/doxygen/compounddef[@kind = 'struct'][(string-length(normalize-space(.)) -  string-length(normalize-space(compoundname))) &gt; 0]"/>
    <xsl:variable name="unions" select="/doxygen/compounddef[@kind = 'union'][(string-length(normalize-space(.)) -  string-length(normalize-space(compoundname))) &gt; 0]"/>
    <xsl:variable name="groups" select="/doxygen/compounddef[@kind = 'group'][(string-length(normalize-space(.)) -  string-length(normalize-space(compoundname))) &gt; 0]"/>
    ...
    <xsl:template name="pmc_iso_element">
        <xsl:element name="bookmeta">
            <pmc_iso audience="PMCInternal">
                ...
                <pmc_document_id>PMC-2152338</pmc_document_id>
                <pmc_issue_date>
                   Aug 2015
                </pmc_issue_date>
                <pmc_dev_status dev_status="Preliminary"/>
                <pmc_doc_status doc_status="Draft">
                ...
                <pmc_footertext>
                    <p>Proprietary and Confidential to PMC-Sierra, Inc., and for its customers' internal use.</p>
                </pmc_footertext>
                <pmc_patents>
                    <p>
                ...
                <pmc_revhistory>
                    <pmc_revision >
                        <pmc_rev_number >1</pmc_rev_number>
                        <pmc_date >Aug 2015</pmc_date>
                        <pmc_name >Shang Zhang</pmc_name>
                        <pmc_revdescription>
                            <p >First release.</p>
                        </pmc_revdescription>
                    </pmc_revision>
    ...
    <xsl:template match="doxygen">
        <xsl:element name="bookmap">           
            ...
            <xsl:element name="chapter">                
                <xsl:attribute name="navtitle">Groups</xsl:attribute>
            ...
            <xsl:if test="count($structures) &gt; 0">
            <xsl:element name="appendix">               
                <xsl:attribute name="navtitle">Data Structures</xsl:attribute>
                <!-- Add attributes needed to connect the chapter to a file named after the section ID -->
            ...
            <xsl:element name="appendix">               
                <xsl:attribute name="navtitle">Files</xsl:attribute>
                <!-- Add attributes needed to connect the chapter to a file named after the section ID -->
            ...
            <xsl:if test="count($unions) &gt; 0">
            <xsl:element name="appendix">               
                <xsl:attribute name="navtitle">Unions</xsl:attribute>                
    ...
    <xsl:template name="create_topicref">
        <xsl:choose>        
            <xsl:when test="@id = //compounddef[@kind = 'group']/innergroup/@refid">
                <xsl:message> Could find <xsl:value-of select="@id"/> belonging to any inner group.</xsl:message>
            </xsl:when>
            <xsl:otherwise>
                <xsl:choose>           
                    <xsl:when test="@id"> 
                        ...
                        <xsl:attribute name="href"><xsl:call-template name="id_processing"><xsl:with-param
                            ...
                            /></xsl:with-param></xsl:call-template>.xml</xsl:attribute>
                        <!-- Add attributes needed to connect the chapter to a file named after the section ID -->
                        <xsl:message> Could not find <xsl:value-of select="@id"/> belonging to any inner group.</xsl:message>
                        <xsl:apply-templates/>
                        <xsl:for-each select="innergroup">
                            <xsl:variable name="refid" select="@refid"/>
                            <xsl:call-template name="innergroups"><xsl:with-param name="node" select="//node()[@id = $refid]"/></xsl:call-template>
                        </xsl:for-each>
    ...
    <xsl:template name="innergroups">
        ...
        <xsl:for-each select="$node">
                  <xsl:choose>           
                    <xsl:when test="@id"> <xsl:element name="topicref">
                        <xsl:call-template name="attribute_manager"/>
                        <xsl:call-template name="convert-attributes"/>
                        <xsl:attribute name="href"><xsl:call-template name="id_processing"><xsl:with-param
                            name="link"><xsl:value-of select="@id"
