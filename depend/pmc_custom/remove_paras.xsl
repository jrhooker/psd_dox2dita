...
<xsl:stylesheet xmlns:db="http://docbook.org/ns/docbook" xmlns:xlink="http://www.w3.org/1999/xlink"
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>A script to remove redundant paragraph elements..</xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xsl:template name="copy-topics">
        <xsl:for-each select="$hrefs">
            ...
            <xsl:variable name="href">/tmp/out/<xsl:value-of select="@href"/></xsl:variable>
            <xsl:message>
                <xsl:value-of select="$href"/>
            </xsl:message>
    ...
    <xsl:template match="p" priority="0" mode="copy">
        <xsl:choose>
